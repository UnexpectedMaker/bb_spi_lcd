...
// No DMA requested or available, fall through to here
#ifdef __LINUX__
    {
        ...
        while (iLen)
        {
            i = iLen;
            if (i > 4096)
                i = 4096; // max default buffer size on Linux SPI driver
...
int spilcdInit(SPILCD *pLCD, int iType, int iFlags, int32_t iSPIFreq, int iCS, int iDC, int iReset, int iLED, int iMISOPin, int iMOSIPin, int iCLKPin, int bUseDMA)
{
    ...
    { // no DMA
        ...
        if (iSPIFreq == -1)
        {                               // externally initialized SPI bus
...
void spilcdCopyClipped(SPILCD *pLCD, int x, int y, int w, int h, uint16_t *pixels)
{
    ...
    if (x + dw > pLCD->iCurrentWidth)
        dw = pLCD->iCurrentWidth - x;
    if (y + dh > pLCD->iCurrentHeight)
        dh = pLCD->iCurrentHeight - y;
...
int spilcdWriteStringCustom(SPILCD *pLCD, GFXfont *pFont, int x, int y, char *szMsg, int usFGColor, int usBGColor, int bBlank, int iFlags)
{
    ...
    while (szMsg[i] && x < pLCD->iCurrentWidth)
    {
        ...
        else if (usFGColor == usBGColor || usBGColor == -1)
        {               // transparent
            ...
            for (ty = 0; ty < cy; ty++)
            {
                for (tx = 0; tx < pGlyph->width; tx++)
                {
                    ...
                    if (tx < cx)
                    {
                        ...
                        { // any opaque pixels to write?
                            if (iCount)
                            { // send this line segment to the display
                ...
                if (iCount)
                { // draw any visible pixels left on the line
                    spilcdSetPosition(pLCD, dx + tx - iCount, dy + ty, iCount, 1, iFlags);
                    myspiWrite(pLCD, ucRXBuf, iCount * sizeof(uint16_t), MODE_DATA, iFlags);
                    iCount = 0;
                }
...
void BB_SPI_LCD::byteSwap(uint16_t *pSrc, uint16_t *pDest, int iPixelCount)
{
    ...
    if (iPixelCount >= 16 && (((intptr_t)pSrc & 15) == ((intptr_t)pDest & 15)))
    { // only worth doing if a large group
...
void BB_SPI_LCD::maskedTint(BB_SPI_LCD *pSrc, BB_SPI_LCD *pMask, int x, int y, uint16_t u16Tint, uint8_t u8Alpha)
{
    ...
    if (!_lcd.pBackBuffer || !pSrc || !pMask || !pSrc->_lcd.pBackBuffer || !pMask->_lcd.pBackBuffer)
        return; // no valid memory planes

#ifdef CONFIG_IDF_TARGET_ESP32S3
    // If the source and destination are the same size, use SIMD code
    if (_lcd.iCurrentWidth == pSrc->_lcd.iCurrentWidth && _lcd.iCurrentHeight == pSrc->_lcd.iCurrentHeight)
    { // use SIMD code
    ...
    for (ty = 0; ty < h; ty++)
    {
        ...
        {
            if (m[0])
            { // N.B. Should check for 0xffff instead of just non-zero
...
// The user has chosen no fallback, so set the pixels to magenta and leave
        ...
        if (y > 1)
        { // copy the blurred pixels back to the source image after we no longer need them
...
int BB_SPI_LCD::drawSprite(int x, int y, BB_SPI_LCD *pSprite, float fScale, int iTransparentColor, int iFlags)
{
    ...
    if (cx < 1 || cy < 1)
        return BB_ERROR_SUCCESS; // too small to be visible
    ...
    if (x < 0)
    {
        ...
        x = 0;
        if (cx < 1)
            return BB_ERROR_SUCCESS; // not visible
    }
    if (y < 0)
    {
...
{
    if (!_lcd.pBackBuffer || x < 0 || y < 0 || x >= _lcd.iCurrentWidth || y >= _lcd.iCurrentHeight)
        return 0;
...
{
    if (_lcd.iLCDType == LCD_VIRTUAL_MEM && (x < 0 || y < 0 || x + w > _lcd.iCurrentWidth || x + h > _lcd.iCurrentHeight))
    { // Special case for clipping of RAM buffers
